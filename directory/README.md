...
**Do NOT put workflow execution inside `directory`** — it answers "who owns this?"
and "where does escalation go?", nothing more.

## Open design points (later)

- Boundary vs the `agent` runtime (directory = structure and authority, agent = behavior).
- Dynamic hiring / org changes at runtime.

## Imports

Use `coactra.directory.repository` for persistence contracts and backends. The older
`coactra.directory.store` and `coactra.directory.sqlite_store` module paths remain
...
`coactra-directory[postgres]`. `TenantOrgStoreRouter` selects a distinct physical
...
Python SDK when `coactra-directory[openfga]` is installed.
