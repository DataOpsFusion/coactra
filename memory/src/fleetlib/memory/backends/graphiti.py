...
from fleetlib.memory.backends.base import event_text
from fleetlib.memory.capabilities import Capability
from fleetlib.memory.export import ExportReport
    ...
    async def remember(self, events: Sequence[MemoryEvent], scope: Scope) -> None:
        ...
        for i, event in enumerate(events):
            ...
            await self._client.add_episode(
                ...
                episode_body=text,
                source_description="fleetlib.memory",
                reference_time=now,
                group_id=gid,
            )
    ...
    async def ingest(self, items: Sequence[Recollection], scope: Scope) -> ExportReport:
        ...
        for i, item in enumerate(items):
            ...
            await self._client.add_episode(
                ...
                source_description="fleetlib.memory:export",
                reference_time=now,
                group_id=gid,
            )
            written += 1
