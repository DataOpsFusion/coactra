# coactra-orchestration work orders
...
pip install 'coactra-orchestration[dbos]'
pip install 'coactra-orchestration[temporal]'
pip install 'coactra-orchestration[dapr]'
pip install 'coactra-orchestration[fsspec]'
...
The stable `coactra.orchestration.work` vocabulary is suitable for a future MCP task adapter, but the
...
```bash
pip install 'coactra-orchestration[sql]' psycopg[binary]
```
