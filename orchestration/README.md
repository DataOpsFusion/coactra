...
pip install 'coactra-orchestration[langgraph]'
pip install 'coactra-orchestration[integrations]'
```
...
```python
from coactra.orchestration import Orchestrator, WorkOrder, WorkScope
...
```text
coactra.orchestration.work       # durable real-job lifecycle and adapters
coactra.orchestration.workflow   # reusable procedures, handlers, stores, engines
coactra.orchestration            # small facade that can link the two
```
...
The wheel continues to ship `coactra.work` and `coactra.workflow` as thin compatibility
aliases. New code should import from `coactra.orchestration`,
`coactra.orchestration.work`, or `coactra.orchestration.workflow`.

Existing `coactra.work.*` audit event names remain stable across the package move.
