...
from coactra.work import AuditContext, Scope, WorkManager, WorkOrder
from coactra.work.adapters import MCPTaskNotTerminalError, MCPTasksAdapter, to_mcp_task


def test_plan_execute_inspect_returns_stable_receipt_and_audit_context():
    manager = WorkManager()
    scope = Scope(tenant_id="acme", namespace="engineering")
    plan = manager.plan(
        scope=scope,
