...
def test_sql_store_create_and_load_work_order(tmp_path):
    ...
    scope = Scope(tenant_id="acme", namespace="agent:build")
...
def test_sql_store_idempotency_key_is_scoped(tmp_path):
    store = make_store(tmp_path)
    manager = WorkManager(store=store)
    acme = Scope(tenant_id="acme", namespace="agent:build")
    globex = Scope(tenant_id="globex", namespace="agent:build")
