...
def test_full_pause_resume_completion_lifecycle_with_artifact_and_events():
    ...
    assert audit.events[-1].type == "coactra.work.completed"
...
def test_expired_deadline_rejects_claim_and_marks_work_failed():
    ...
    assert manager.events(order.id, scope)[-1].type == "coactra.work.deadline_exceeded"
...
def test_usage_budget_stops_running_work_and_releases_lease():
    ...
    assert manager.events(order.id, scope)[-1].type == "coactra.work.budget_exceeded"
