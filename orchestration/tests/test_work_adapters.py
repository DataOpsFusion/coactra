...
from coactra.work.adapters import (
    DaprDispatcher,
    DBOSDispatcher,
    FsspecArtifactStore,
...
def test_cloudevents_converter_uses_sdk_constructor(monkeypatch):
    from coactra.work.adapters import cloudevents
    ...
    converted = to_cloudevent(EventEnvelope(type="coactra.work.started", tenant_id="acme"))
...
def test_opentelemetry_sink_emits_lifecycle_span_and_scalar_event_attributes():
    ...
    sink.emit(
        EventEnvelope(
            type="coactra.work.completed",
            tenant_id="acme",
            subject="work-1",
            data={"status": "completed", "nested": {"drop": True}},
        )
    )
    assert tracer.calls[0][0] == "coactra.work.completed"
    assert tracer.span.events == [("coactra.work.completed", {"status": "completed"})]
