...
class AIPort(Protocol):
    def ask(self, prompt: str) -> str:
        """Free-text model completion (mirrors coactra.ai.ask)."""
        ...

    def structured(self, schema: type[T], prompt: str) -> T:
        """Typed model output validated against `schema` (mirrors coactra.ai.structured)."""
        ...
...
class MemoryPort(Protocol):
    async def remember(self, events: Sequence[Any], scope: Scope) -> None:
        """Persist conversational events within scope (mirrors coactra.memory.remember)."""
        ...
