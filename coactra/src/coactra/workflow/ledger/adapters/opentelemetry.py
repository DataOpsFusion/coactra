...
class OpenTelemetryAuditSink:
    ...
    def __init__(self, tracer: Any | None = None) -> None:
        if tracer is None:
            ...
            tracer = trace.get_tracer("coactra.workflow")
        self.tracer = tracer

    def emit(self, event: EventEnvelope) -> None:
        attributes = {
            "coactra.workflow.event_id": event.id,
            "coactra.workflow.tenant_id": event.tenant_id,
        }
        if event.subject is not None:
            attributes["coactra.workflow.id"] = event.subject
