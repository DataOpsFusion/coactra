...
def test_dbos_dispatcher_maps_work_order_to_registered_queue():
    ...
    order = WorkOrder(
        id="work-1",
        scope=WorkScope(tenant_id="acme", namespace="ops"),
...
async def test_temporal_dispatcher_uses_work_id_and_task_queue():
    ...
    order = WorkOrder(id="work-1", scope=WorkScope(tenant_id="acme"), title="Process incident")
...
def test_cloudevents_converter_uses_sdk_constructor(monkeypatch):
    from coactra.workflow.ledger.adapters import cloudevents
    ...
    converted = to_cloudevent(EventEnvelope(type="coactra.workflow.started", tenant_id="acme"))
...
def test_opentelemetry_sink_emits_lifecycle_span_and_scalar_event_attributes():
    ...
    sink.emit(
        EventEnvelope(
            type="coactra.workflow.completed",
            tenant_id="acme",
            subject="work-1",
            data={"status": "completed", "nested": {"drop": True}},
        )
    )
    assert tracer.calls[0][0] == "coactra.workflow.completed"
    assert tracer.span.events == [("coactra.workflow.completed", {"status": "completed"})]
...
def test_fsspec_artifact_store_is_scope_isolated():
    ...
    with pytest.raises(ValueError):
        store.get(reference, WorkScope(tenant_id="globex", namespace="support"))
