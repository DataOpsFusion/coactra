...
def test_full_pause_resume_completion_lifecycle_with_artifact_and_events():
    ...
    assert audit.events[-1].type == "coactra.workflow.completed"
...
def test_scope_isolation_applies_to_orders_and_events():
    ...
    acme = WorkScope(tenant_id="acme", namespace="private")
    public = WorkScope(tenant_id="acme", namespace="public")
...
def test_expired_deadline_rejects_claim_and_marks_work_failed():
    ...
    assert manager.events(order.id, scope)[-1].type == "coactra.workflow.deadline_exceeded"
...
def test_usage_budget_stops_running_work_and_releases_lease():
    ...
    assert manager.events(order.id, scope)[-1].type == "coactra.workflow.budget_exceeded"
