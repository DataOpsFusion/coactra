...
def test_structured_uses_instructor_response_model():
    ...
    with patch("coactra.ai.completion.client.instructor.from_litellm", return_value=fake_client):
...
def test_structured_tools_falls_back_to_json_on_tool_choice_error():
    ...
    with patch("coactra.ai.completion.client.instructor.from_litellm", side_effect=from_litellm):
...
def test_make_completer_binds_api_base_and_key():
    ...
    with patch("coactra.ai.completion.client.litellm.completion", side_effect=fake_completion):
...
def test_client_structured_threads_config_to_standalone_structured():
    ...
    with patch("coactra.ai.completion.client.structured") as fake_structured:
