"""Organization-backed authorization for memory writes."""
...
def write_action(tenant: str, agent: str) -> str:
    """Return the legacy organization permission token for an agent memory write."""
    return f"memory:write:{tenant}:{agent}"


def scope_write_action(scope: Any) -> str:
    """Return the organization permission token for an exact memory scope.
...
class MemoryAcl:
    """Resolve memory write permission through an organization facade."""
    ...
    def check_write(self, agent_name: str, scope: Any) -> None:
        """Permit a scoped write or fail closed with :class:`ScopeViolation`."""
        action = scope_write_action(scope)
        member = self._members.get(agent_name)
        if member is None:
            raise ScopeViolation(
                f"agent {agent_name!r} has no organization membership; "
    ...
    def for_scopes(
        ...
        *,
        ...
        """Seed a minimal organization granting an agent an explicit scope allowlist."""
    ...
    def for_own_scope(
        ...
        *,
        ...
        """Seed a minimal organization granting only the agent's legacy own scope."""
