...
- The default durable workflow engine can pause/resume inside the same process; process-restart durability requires an injected host runtime or store-backed engine.
...
- Memory persistence is backend-specific under `coactra/src/coactra/memory/backends/`.
