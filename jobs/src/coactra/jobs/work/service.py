...
from coactra.jobs.work.domain.events import EventEnvelope, utc_now
...
from coactra.jobs.work.store import AuditSink, ConflictError, WorkStore
...
class WorkManager:
    ...
    def claim(self, work_id: str, scope: Scope, *, worker: str, lease_seconds: int = 300) -> Lease:
        ...
        if order.deadline is not None and order.deadline.expired(at=self.clock()):
            ...
            self._save_and_emit(order, "coactra.jobs.deadline_exceeded")
        ...
        order.lease = lease
        saved = self._save_and_emit(order, "coactra.jobs.claimed", worker=worker, lease_id=lease.id)
    ...
    def heartbeat(self, lease: Lease, scope: Scope, *, lease_seconds: int = 300) -> Lease:
        ...
        saved = self._save_and_emit(order, "coactra.jobs.heartbeat", lease_id=current.id)
        return saved.lease  # type: ignore[return-value]
    ...
    def start(self, lease: Lease, scope: Scope) -> WorkOrder:
        ...
        return self._save_and_emit(order, "coactra.jobs.started", attempt=attempt_number)
    ...
    def checkpoint(self, lease: Lease, scope: Scope, state: dict[str, Any]) -> ResumeToken:
        ...
        self._save_and_emit(order, "coactra.jobs.checkpointed", resume_token=token.value)
        return token
    ...
    def resume(self, work_id: str, scope: Scope, *, token: ResumeToken | None = None) -> WorkOrder:
        ...
        order.lease = None
        order.assignment = None
        return self._save_and_emit(order, "coactra.jobs.resumed")
    ...
    def record_usage(
        ...
        *,
        ...
        if order.budget is not None and not order.budget.allows(
            ...
            order.lease = None
            order.assignment = None
            return self._save_and_emit(order, "coactra.jobs.budget_exceeded")
        return self._save_and_emit(
            order,
            "coactra.jobs.usage_recorded",
    ...
    def complete(
        ...
        order.lease = None
        order.assignment = None
        return self._save_and_emit(order, "coactra.jobs.completed", artifact_ids=[a.id for a in artifacts])
    ...
    def fail(self, lease: Lease, scope: Scope, *, error: str, retry: bool = True) -> WorkOrder:
        ...
        if retry and len(order.attempts) < self._attempts_allowed(order):
            order.status = WorkStatus.queued
            event_type = "coactra.jobs.retry_scheduled"
        else:
            order.status = WorkStatus.failed
            event_type = "coactra.jobs.failed"
    ...
    def cancel(self, work_id: str, scope: Scope, *, reason: str = "") -> WorkOrder:
        ...
        order.assignment = None
        order.pending_request = None
        return self._save_and_emit(order, "coactra.jobs.cancelled", reason=reason)
    ...
    def reap_stale(self, scope: Scope, *, retry: bool = True) -> list[WorkOrder]:
        ...
        for order in self.list(scope):
            ...
            if order.deadline is not None and order.deadline.expired(at=now):
                ...
                order.lease = None
                order.assignment = None
                event_type = "coactra.jobs.deadline_exceeded"
            elif (
                ...
                if retry and len(order.attempts) < self._attempts_allowed(order):
                    order.status = WorkStatus.queued
                    event_type = "coactra.jobs.stale_requeued"
                else:
                    order.status = WorkStatus.failed
                    event_type = "coactra.jobs.stale_failed"
    ...
    def _pause(
        ...
        *,
        ...
        order.lease = None
        order.assignment = None
        return self._save_and_emit(order, f"coactra.jobs.{status.value}", request_id=request.id)
